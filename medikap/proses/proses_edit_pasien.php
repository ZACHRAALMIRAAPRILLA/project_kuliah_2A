    ...
    if ($statusUpload == 0) {
        ...
        if (mysqli_num_rows($select) > 0) {
            ...
            if (move_uploaded_file($_FILES['foto']['tmp_name'], $target_file)) {
                $query = mysqli_query($conn, "UPDATE tb_riwayat_pasien SET foto='" . $kode_rand. $_FILES['foto']['name'] . "', nama_pasien='$nama_pasien', keterangan='$keterangan', kategori='$kat_pasien', hasil='$hasil' WHERE id='$id'");
                if ($query) {
